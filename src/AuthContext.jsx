...
export function AuthProvider({ children }) {
  ...
  const uploadItemImage = useCallback(async (itemId, base64) => {
    if (!user) return null;
    try {
      // Convert base64 to blob
      const res = await fetch(base64);
      const blob = await res.blob();
      const r = ref(storage, `items/${user.uid}/${itemId}.jpg`);
      await uploadBytes(r, blob);
      return await getDownloadURL(r);
    } catch (e) {
      console.warn("Upload item image:", e);
      return null;
    }
  }, [user]);

  const deleteItemImage = useCallback(async (itemId) => {
    if (!user) return;
    try {
      const r = ref(storage, `items/${user.uid}/${itemId}.jpg`);
  ...
  const deleteAllItemImages = useCallback(async () => {
    if (!user) return;
    try {
      const folderRef = ref(storage, `items/${user.uid}`);
      const list = await listAll(folderRef);
      await Promise.all(list.items.map(item => deleteObject(item)));
    } catch (e) {
      console.warn("Delete all item images:", e);
    }
  }, [user]);
  ...
  const value = {
    ...
    saveProfile, saveSettings, uploadPhoto, uploadItemImage, deleteItemImage,
    syncItems, loadItems,
