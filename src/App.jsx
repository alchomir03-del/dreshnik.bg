  ...
  return(<div style={{paddingBottom:90}}>
    ...
    <div style={{padding:"14px 14px 0"}}>
      <div style={{display:"flex",gap:6,marginBottom:12,flexWrap:"wrap"}}>
        <button onClick={()=>setShowCal(true)} style={{padding:"8px 14px",borderRadius:20,border:"1px solid "+T.border,background:"transparent",fontFamily:F.sans,fontSize:11,color:T.textMuted,cursor:"pointer"}}>{t("Календар")}</button>
        <button onClick={()=>setHideSeasonal(!hideSeasonal)} style={{padding:"8px 14px",borderRadius:20,border:hideSeasonal?"1px solid "+T.text:"1px solid "+T.border,background:hideSeasonal?T.glow:"transparent",fontFamily:F.sans,fontSize:11,color:hideSeasonal?T.text:T.textMuted,cursor:"pointer"}}>{t("Сезонен")}</button>
        <button onClick={()=>{setBatchMode(!batchMode);setSelected(new Set());}} style={{padding:"8px 14px",borderRadius:20,border:batchMode?"1px solid "+T.text:"1px solid "+T.border,background:batchMode?T.glow:"transparent",fontFamily:F.sans,fontSize:11,color:batchMode?T.text:T.textMuted,cursor:"pointer"}}>{t("Пералня")}</button>
      </div>
      {batchMode&&<div style={{display:"flex",gap:6,marginBottom:12,padding:"10px 14px",background:T.card,border:"1px solid "+T.borderLight,borderRadius:3}}>
        <span style={{fontFamily:F.sans,fontSize:11,color:T.textMuted,flex:1,alignSelf:"center"}}>{selected.size} {t("избрани")}</span>
        <button onClick={()=>batchLaundry(true)} disabled={!selected.size} style={{padding:"8px 12px",borderRadius:20,border:"1px solid "+T.border,background:"transparent",fontFamily:F.sans,fontSize:10,color:T.textMuted,cursor:"pointer",opacity:selected.size?1:0.3}}>{t("В пералня")}</button>
        <button onClick={()=>batchLaundry(false)} disabled={!selected.size} style={{padding:"8px 12px",borderRadius:20,border:"1px solid "+T.border,background:"transparent",fontFamily:F.sans,fontSize:10,color:T.textMuted,cursor:"pointer",opacity:selected.size?1:0.3}}>{t("Чисти")}</button>
      </div>}
      <input value={search} onChange={e=>setSearch(e.target.value)} placeholder={t("Търси по име, марка, таг...")} style={{width:"100%",padding:"12px 16px",borderRadius:2,border:"1px solid "+T.border,fontSize:12,fontFamily:F.sans,color:T.text,background:T.surface,boxSizing:"border-box",marginBottom:12}}/>
...
function AddTab({onAdd,lastCat}){
  ...
  if(step==="photo"){return(<div style={{padding:"24px 20px"}}><p style={{...labelSt,marginBottom:3}}>{t("НОВА ДРЕХА")}</p><h2 style={{fontFamily:F.serif,fontSize:26,fontWeight:300,color:T.text,margin:"0 0 28px"}}>{t("Добави в гардероба")}</h2>
...
function SavedTab({saved,items,onDelete}){
  ...
  return(<div style={{padding:"24px 20px 100px"}}><p style={{fontFamily:F.mono,fontSize:10,color:T.textDim,letterSpacing:"0.16em",marginBottom:3}}>{t("КОЛЕКЦИЯ")}</p><h2 style={{fontFamily:F.serif,fontSize:26,fontWeight:300,color:T.text,margin:"0 0 14px"}}>{t("Запазени визии")}</h2>
    <div style={{display:"flex",gap:6,marginBottom:16}}>{[{id:"date",l:t("По дата")},{id:"score",l:t("По оценка")},{id:"occasion",l:t("По повод")}].map(s=><Pill key={s.id} active={sortBy===s.id} onClick={()=>setSortBy(s.id)} small>{s.l}</Pill>)}</div>
...
function InsightsTab({items,saved}){
  ...
  return(<div style={{padding:"24px 20px 100px"}}><p style={{fontFamily:F.mono,fontSize:10,color:T.textDim,letterSpacing:"0.16em",marginBottom:3}}>{t("АНАЛИЗ НА ГАРДЕРОБА")}</p><h2 style={{fontFamily:F.serif,fontSize:26,fontWeight:300,color:T.text,margin:"0 0 22px"}}>{t("Статистика")}</h2>
...
function AuthScreen(){
  ...
  return(<div style={{minHeight:"100dvh",background:T.bg,display:"flex",flexDirection:"column",justifyContent:"center",padding:"0 28px"}}>
    <div style={{marginBottom:44}}><Logo size={32}/><p style={{fontFamily:F.sans,fontSize:14,color:T.textMuted,margin:"8px 0 0"}}>{mode==="login"?t("Добре дошъл обратно"):mode==="register"?t("Създай своя гардероб"):t("Възстанови парола")}</p></div>
