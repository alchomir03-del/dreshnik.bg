...
function Logo({size=28,animate}){const T=useTheme();return(<span style={{fontFamily:F.serif,fontSize:size,color:T.text,animation:animate?"pulse 2s ease-in-out infinite":"none",letterSpacing:"-0.02em"}}><span style={{fontWeight:500}}>DRESHNIK</span><span style={{fontWeight:300,color:T.textDim}}>.bg</span></span>);}
...
function WeatherBar({weather}){const T=useTheme(),t=useT();if(!weather)return null;const desc=t(weather.desc);return(<div style={{margin:"14px 20px 0",display:"inline-flex",alignItems:"center",gap:10,padding:"10px 18px",background:T.pillBg,border:"1px solid "+T.pillBorder,borderRadius:100}}><span style={{fontSize:16}}>{weather.icon}</span><span style={{fontFamily:F.sans,fontSize:12,color:T.textMuted}}>{weather.temp}° {desc}{weather.city?" / "+weather.city:""}</span>{weather.rainChance>0&&<span style={{fontFamily:F.mono,fontSize:10,color:T.textDim}}>☂ {weather.rainChance}%</span>}</div>);}
function EmptyState({icon,title,sub}){const T=useTheme();return(<div style={{textAlign:"center",padding:"60px 24px"}}><div style={{fontSize:32,marginBottom:20,opacity:0.3}}>{icon}</div><p style={{fontFamily:F.serif,fontSize:22,color:T.text,fontWeight:300,marginBottom:8,letterSpacing:"-0.01em"}}>{title}</p><p style={{fontFamily:F.mono,fontSize:10,color:T.textDim,letterSpacing:"0.16em",textTransform:"uppercase"}}>{sub}</p></div>);}
  ...
  return(<div style={{paddingBottom:90}}>
    ...
    <div style={{padding:"20px 20px 0"}}>
      ...
      <div style={{display:"flex",gap:8,marginBottom:16}}>
        <button onClick={()=>setShowCal(true)} style={{padding:"8px 16px",borderRadius:100,border:"1px solid "+T.pillBorder,background:T.pillBg,fontFamily:F.serif,fontSize:12,color:T.textMuted,cursor:"pointer",fontWeight:300,letterSpacing:"0.02em",transition:"all 0.3s"}}>{t("Календар")}</button>
        <button onClick={()=>setHideSeasonal(!hideSeasonal)} style={{padding:"8px 16px",borderRadius:100,border:hideSeasonal?"1px solid "+T.text:"1px solid "+T.pillBorder,background:T.pillBg,fontFamily:F.serif,fontSize:12,color:hideSeasonal?T.text:T.textMuted,cursor:"pointer",fontWeight:hideSeasonal?500:300,letterSpacing:"0.02em",transition:"all 0.3s"}}>{t("Сезонен")}</button>
        <button onClick={()=>{setBatchMode(!batchMode);setSelected(new Set());}} style={{padding:"8px 16px",borderRadius:100,border:batchMode?"1px solid "+T.text:"1px solid "+T.pillBorder,background:T.pillBg,fontFamily:F.serif,fontSize:12,color:batchMode?T.text:T.textMuted,cursor:"pointer",fontWeight:batchMode?500:300,letterSpacing:"0.02em",transition:"all 0.3s"}}>{t("Пералня")}</button>
      </div>
      {batchMode&&<div style={{display:"flex",gap:8,marginBottom:14,padding:"12px 16px",background:T.subtleBg,borderLeft:"2px solid "+T.text,alignItems:"center"}}>
        <span style={{fontFamily:F.serif,fontSize:13,color:T.textSoft,flex:1,fontWeight:300}}>{selected.size} {t("избрани")}</span>
        <button onClick={()=>batchLaundry(true)} disabled={!selected.size} style={{padding:"6px 14px",borderRadius:100,border:"1px solid "+T.pillBorder,background:T.pillBg,fontFamily:F.serif,fontSize:11,color:T.textMuted,cursor:"pointer",opacity:selected.size?1:0.3}}>{t("В пералня")}</button>
        <button onClick={()=>batchLaundry(false)} disabled={!selected.size} style={{padding:"6px 14px",borderRadius:100,border:"1px solid "+T.pillBorder,background:T.pillBg,fontFamily:F.serif,fontSize:11,color:T.textMuted,cursor:"pointer",opacity:selected.size?1:0.3}}>{t("Чисти")}</button>
      </div>}
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(152px,1fr))",gap:9}}>{filtered.map((item,i)=><div key={item.id} style={{position:"relative"}}>{batchMode&&<div style={{position:"absolute",top:6,right:6,zIndex:10,width:22,height:22,borderRadius:"50%",border:"2px solid "+(selected.has(item.id)?T.text:T.border),background:selected.has(item.id)?T.text:T.subtleBg,display:"flex",alignItems:"center",justifyContent:"center",pointerEvents:"none"}}>{selected.has(item.id)&&<span style={{color:T.bg,fontSize:12}}>✓</span>}</div>}<ItemCard item={item} idx={i} onClick={()=>handleItemClick(item)} onDoubleTap={batchMode?toggleSelect:toggleFav}/></div>)}</div>}
    </div>
...
function AddTab({onAdd,lastCat}){
  ...
  return(<div style={{padding:"28px 20px 100px"}}>
    ...
    <div style={{marginBottom:22}}><label style={labelSt}>{t("ЦВЕТОВЕ * — ДО 3")}</label><div style={{display:"flex",flexWrap:"wrap",gap:5}}>{COLORS.map(([hex])=><button key={hex} onClick={()=>toggleColor(hex)} style={{width:32,height:32,borderRadius:3,background:hex,cursor:"pointer",padding:0,border:colors.includes(hex)?"2px solid "+T.text:hex==="#FFFFFF"?"1px solid "+T.border:"1px solid transparent",transform:colors.includes(hex)?"scale(1.15)":"scale(1)",transition:"all 0.2s cubic-bezier(0.16, 1, 0.3, 1)"}}/>)}</div>{colors.length>0&&<div style={{display:"flex",gap:4,marginTop:8}}>{colors.map(c=><span key={c} style={{fontFamily:F.mono,fontSize:10,border:"1px solid "+T.pillBorder,padding:"3px 10px",borderRadius:100,color:T.textMuted,letterSpacing:"0.06em",textTransform:"uppercase"}}>{getColorName(c)}</span>)}</div>}</div>
    ...
    <div style={{display:"flex",gap:10,marginBottom:24}}><div style={{flex:1}}><label style={labelSt}>{t("ЦЕНА")}</label><input type="number" value={price} onChange={e=>setPrice(e.target.value)} placeholder="0" style={{...inputSt,fontSize:12}}/></div><div style={{flex:2}}><label style={labelSt}>{t("ТАГОВЕ — ДО 5")}</label><div style={{display:"flex",gap:6}}><input value={tagIn} onChange={e=>setTagIn(e.target.value)} onKeyDown={e=>e.key==="Enter"&&addTag()} placeholder="#лято" style={{...inputSt,fontSize:12,flex:1}}/><button onClick={addTag} style={{padding:"0 16px",borderRadius:100,border:"1px solid "+T.pillBorder,background:T.pillBg,color:T.textMuted,cursor:"pointer",fontFamily:F.mono,fontSize:10}}>+</button></div>{tags.length>0&&<div style={{display:"flex",gap:4,marginTop:7,flexWrap:"wrap"}}>{tags.map(t=><span key={t} onClick={()=>setTags(p=>p.filter(x=>x!==t))} style={{fontFamily:F.mono,fontSize:10,color:T.textMuted,background:T.pillBg,padding:"4px 10px",borderRadius:100,cursor:"pointer",border:"1px solid "+T.pillBorder}}>#{t} ×</span>)}</div>}</div></div>
...
function InsightsTab({items,saved}){
  ...
  return(<div style={{padding:"28px 20px 100px"}}><p style={{fontFamily:F.mono,fontSize:10,letterSpacing:"0.3em",color:T.textDim,marginBottom:8}}>{t("АНАЛИЗ НА ГАРДЕРОБА")}</p><h2 style={{fontFamily:F.serif,fontSize:34,fontWeight:300,color:T.text,margin:"0 0 6px",letterSpacing:"-0.02em"}}>{t("Статистика")}</h2><div style={{width:40,height:1,background:"linear-gradient(90deg, "+T.text+" 0%, transparent 100%)",marginBottom:24}}/>
    <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(80px,1fr))",gap:6,marginBottom:18}}>{[{n:items.length,l:t("ДРЕХИ")},{n:totalWears,l:t("НОСЕНИЯ")},{n:saved.length,l:t("ВИЗИИ")},{n:totalValue>0?totalValue+cur:"—",l:t("СТОЙНОСТ")}].map((d,i)=><div key={i} style={{background:T.subtleBg,border:"1px solid "+T.border,borderRadius:4,padding:"18px 8px",textAlign:"center"}}><p style={{fontFamily:F.serif,fontSize:26,color:T.text,margin:"0 0 3px",fontWeight:300}}>{d.n}</p><p style={{fontFamily:F.mono,fontSize:10,color:T.textDim,letterSpacing:"0.14em",margin:0}}>{d.l}</p></div>)}</div>
    {/* Achievements */}
    <Section title={t("🏆 ПОСТИЖЕНИЯ")}>{achievements.map((a,i)=><div key={i} style={{display:"flex",alignItems:"center",gap:10,padding:"8px 0",borderBottom:i<achievements.length-1?"1px solid "+T.border:"none",opacity:a.done?1:0.4}}><span style={{fontSize:18}}>{a.icon}</span><div style={{flex:1}}><p style={{fontFamily:F.sans,fontSize:11,color:T.text,margin:0}}>{a.name}</p><p style={{fontFamily:F.mono,fontSize:10,color:T.textDim,margin:"2px 0 0"}}>{a.desc}</p></div><span style={{fontFamily:F.mono,fontSize:11,color:a.done?T.successText:T.textDim}}>{a.done?"✓":a.progress}</span></div>)}</Section>
...
export default function App(){
  ...
  return(<ThemeCtx.Provider value={T}><ToastProvider><ConfirmProvider><div style={{maxWidth:600,margin:"0 auto",minHeight:"100dvh",background:T.bg,paddingBottom:"env(safe-area-inset-bottom, 0px)",transition:"background 0.3s"}}><style>{globalStyles}</style>
