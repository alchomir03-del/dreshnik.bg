...
function ProfileTab(){
  ...
  return(<div style={{padding:"24px 20px 100px"}}>
    ...
    <Section title="НАСТРОЙКИ">
      ...
      <div style={{padding:"12px 0",borderBottom:"1px solid "+T.border}}>
        <p style={{fontFamily:F.mono,fontSize:7,color:T.textDim,letterSpacing:"0.12em",marginBottom:8}}>ЕЗИК</p>
        <div style={{display:"flex",gap:6}}>{[{id:"bg",label:"Български"},{id:"en",label:"English"}].map(l=><Pill key={l.id} active={(settings.language||"bg")===l.id} onClick={()=>saveSettings({language:l.id})} small>{l.label}</Pill>)}</div>
      </div>
    </Section>
    ...
    <p style={{fontFamily:F.mono,fontSize:8,color:T.textDim,textAlign:"center",letterSpacing:"0.08em",marginTop:20}}>DRESHNIK.BG v8 · MADE IN BULGARIA 🇧🇬</p>
  </div>);
}
