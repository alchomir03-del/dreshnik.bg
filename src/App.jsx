...
function InsightsTab({items,saved}){
  ...
  return(<div style={{padding:"24px 20px 100px"}}><p style={{fontFamily:F.mono,fontSize:10,color:T.textDim,letterSpacing:"0.16em",marginBottom:3}}>АНАЛИЗ НА ГАРДЕРОБА</p><h2 style={{fontFamily:F.serif,fontSize:26,fontWeight:300,color:T.text,margin:"0 0 22px"}}>Статистика</h2>
    ...
    {totalValue>0&&<Section title="РАЗХОДИ ПО КАТЕГОРИЯ">{catBreakdown.filter(c=>c.value>0).map(c=><div key={c.id} style={{marginBottom:8}}><div style={{display:"flex",justifyContent:"space-between",marginBottom:3}}><span style={{fontFamily:F.sans,fontSize:11,color:T.textSoft}}>{c.label}</span><span style={{fontFamily:F.mono,fontSize:11,color:T.textDim}}>{c.value}лв ({c.count})</span></div><div style={{height:3,background:T.border,borderRadius:2,overflow:"hidden"}}><div style={{width:(c.value/totalValue*100)+"%",height:"100%",background:T.text,borderRadius:2}}/></div></div>)}<div style={{marginTop:10,paddingTop:10,borderTop:"1px solid "+T.border}}><div style={{display:"flex",justifyContent:"space-between"}}><span style={{fontFamily:F.mono,fontSize:11,color:T.textMuted}}>СР. ЦЕНА/НОСЕНЕ</span><span style={{fontFamily:F.serif,fontSize:18,color:T.text}}>{avgCPW} лв</span></div></div></Section>}
    {bestCPW.length>0&&<Section title="НАЙ-ДОБРА ЦЕНА/НОСЕНЕ">{bestCPW.map((item,i)=><div key={item.id} style={{display:"flex",alignItems:"center",gap:10,padding:"6px 0",borderBottom:i<bestCPW.length-1?"1px solid "+T.border:"none"}}>{item.image?<img src={item.image} alt="" style={{width:28,height:28,borderRadius:2,objectFit:"cover"}}/>:<div style={{width:28,height:28,borderRadius:2,background:T.surface}}/>}<span style={{fontFamily:F.sans,fontSize:11,color:T.text,flex:1}}>{item.name}</span><span style={{fontFamily:F.mono,fontSize:11,color:T.textDim}}>{item.cpw}лв/носене</span></div>)}</Section>}
