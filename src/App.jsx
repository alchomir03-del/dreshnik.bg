...
function ToastOverlay({toasts}){
  ...
  return(<div style={{position:"fixed",top:"env(safe-area-inset-top, 12px)",left:0,right:0,zIndex:999,display:"flex",flexDirection:"column",alignItems:"center",gap:6,padding:"12px 20px",pointerEvents:"none"}}>
    {toasts.map(t=><div key={t.id} style={{background:t.type==="error"?T.danger:t.type==="success"?"#112211":T.card,border:"1px solid "+(t.type==="error"?T.dangerText:t.type==="success"?"#336633":T.border),borderRadius:3,padding:"10px 20px",fontFamily:F.mono,fontSize:10,color:t.type==="error"?T.dangerText:t.type==="success"?"#88CC88":T.textSoft,letterSpacing:"0.06em",animation:"fadeInUp 0.25s ease",pointerEvents:"auto",maxWidth:340,textAlign:"center"}}>{t.msg}</div>)}
  </div>);
}
  ...
  return(<div style={{position:"fixed",inset:0,background:T.overlay,backdropFilter:"blur(12px)",zIndex:300,display:"flex",alignItems:"center",justifyContent:"center",padding:24,animation:"fadeIn .15s"}} onClick={onNo}>
    <div onClick={e=>e.stopPropagation()} style={{background:T.surface,border:"1px solid "+T.border,borderRadius:4,padding:24,maxWidth:320,width:"100%",animation:"fadeInUp .2s ease"}}>
      ...
      <div style={{display:"flex",gap:8}}>
        <button onClick={onNo} style={{flex:1,padding:12,borderRadius:2,border:"1px solid "+T.border,background:"transparent",fontFamily:F.mono,fontSize:9,color:T.textMuted,cursor:"pointer",letterSpacing:"0.08em"}}>ОТКАЗ</button>
        <button onClick={onYes} style={{flex:1,padding:12,borderRadius:2,border:"none",background:T.dangerText,color:"#fff",fontFamily:F.mono,fontSize:9,cursor:"pointer",letterSpacing:"0.08em"}}>ПОТВЪРДИ</button>
      </div>
    </div>
  </div>);
}
...
function useWeather(){const[w,setW]=useState(weatherCache);useEffect(()=>{if(Date.now()-weatherTs<600000&&weatherCache){setW(weatherCache);return;}if(!navigator.geolocation)return;navigator.geolocation.getCurrentPosition(async pos=>{try{const lat=pos.coords.latitude,lon=pos.coords.longitude;const r=await fetch(`https://api.open-meteo.com/v1/forecast?latitude=${lat}&longitude=${lon}&current=temperature_2m,weather_code,wind_speed_10m,relative_humidity_2m&daily=temperature_2m_max,temperature_2m_min,weather_code,precipitation_probability_max&timezone=auto&forecast_days=1`);const d=await r.json();const code=d.current?.weather_code||0;const result={temp:Math.round(d.current?.temperature_2m||0),rain:code>=51&&code<=67,code,wind:Math.round(d.current?.wind_speed_10m||0),humidity:d.current?.relative_humidity_2m||0,high:Math.round(d.daily?.temperature_2m_max?.[0]||0),low:Math.round(d.daily?.temperature_2m_min?.[0]||0),rainChance:d.daily?.precipitation_probability_max?.[0]||0,desc:WC[code]||"Ясно",icon:getWIcon(code)};weatherCache=result;weatherTs=Date.now();setW(result);}catch(e){console.warn(e);}},()=>{},{timeout:5000});},[]);return w;}
...
function ItemCard({item,onClick,onDoubleTap,idx=0}){
  ...
  return(<div onClick={handleTap} style={{background:T.card,borderRadius:3,overflow:"hidden",cursor:"pointer",border:"1px solid "+T.border,boxShadow:T.shadow,opacity:item.inLaundry?0.45:1,transition:"all 0.35s cubic-bezier(.23,1,.32,1)",animation:"fadeInUp "+(0.15+idx*0.04)+"s ease both"}}>
    <div style={{position:"relative",paddingTop:"125%",background:T.surface}}>
      ...
      <span style={{position:"absolute",top:8,left:8,background:"rgba(0,0,0,0.55)",backdropFilter:"blur(16px)",padding:"3px 8px",borderRadius:2,fontFamily:F.mono,fontSize:7,color:"rgba(255,255,255,0.5)",letterSpacing:"0.12em",textTransform:"uppercase"}}>{cat?.label}{item.inLaundry?" · 🧺":""}</span>
      ...
      {(item.wearCount||0)>0&&<span style={{position:"absolute",bottom:8,left:8,fontFamily:F.mono,fontSize:8,color:"rgba(255,255,255,0.35)"}}>{item.wearCount}×</span>}
    </div>
    <div style={{padding:"10px 10px 11px"}}>
      ...
      <p style={{fontFamily:F.mono,fontSize:8,color:T.textDim,margin:"3px 0 0",letterSpacing:"0.04em"}}>{item.brand||STYLES[STYLE_IDS.indexOf(item.style)]||""}</p>
    </div>
  </div>);
}
...
function Modal({item,onClose,onDelete,onUpdate}){
  ...
  const labelSt={fontFamily:F.mono,fontSize:7,color:T.textDim,letterSpacing:"0.12em",display:"block",marginBottom:4,textTransform:"uppercase"};
      ...
      <div style={{padding:"20px 22px 34px"}}>
        {!editing?<>
          <div style={{display:"flex",gap:6,marginBottom:10,flexWrap:"wrap"}}>
            ...
            {item.capsule&&<span style={{border:"1px solid #665500",padding:"3px 10px",borderRadius:2,fontFamily:F.mono,fontSize:8,color:"#CCAA00",letterSpacing:"0.08em"}}>КАПСУЛА</span>}
            {item.inLaundry&&<span style={{border:"1px solid #445566",padding:"3px 10px",borderRadius:2,fontFamily:F.mono,fontSize:8,color:"#88AACC",letterSpacing:"0.08em"}}>В ПЕРАЛНЯ</span>}
          </div>
        ...
        <div>
          ...
          <label style={labelSt}>ЦЕНА (ЛВ)</label><input type="number" value={editData.price||""} onChange={e=>setEditData(p=>({...p,price:e.target.value}))} style={inputSt}/>
...
function OOTDCard({items,weather,onDismiss,calendar}){
  ...
  return(<div style={{margin:"14px 14px 0",background:T.card,border:"1px solid "+T.borderLight,borderRadius:4,overflow:"hidden",boxShadow:T.shadow}}>
    <div style={{padding:"16px 18px 14px",display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
      <div><p style={{fontFamily:F.mono,fontSize:8,color:T.textDim,letterSpacing:"0.16em",margin:"0 0 4px"}}>ВИЗИЯ НА ДЕНЯ</p><p style={{fontFamily:F.serif,fontSize:18,color:T.text,fontWeight:400,margin:0}}>{wornToday?"Вече отбеляза днешната визия":"Предложение за днес"}</p></div>
  ...
  return(<div style={{position:"fixed",inset:0,background:T.overlay,backdropFilter:"blur(16px)",zIndex:200,display:"flex",alignItems:"flex-end",justifyContent:"center",animation:"fadeIn .2s"}} onClick={onClose}>
    <div onClick={e=>e.stopPropagation()} style={{background:T.surface,borderRadius:"18px 18px 0 0",width:"100%",maxWidth:500,maxHeight:"80vh",overflow:"auto",animation:"slideUp .35s cubic-bezier(.23,1,.32,1)",padding:"20px 22px 34px"}}>
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(7,1fr)",gap:2,marginBottom:8}}>
        {["Пн","Вт","Ср","Чт","Пт","Сб","Нд"].map(d=><div key={d} style={{textAlign:"center",padding:"4px 0",fontFamily:F.mono,fontSize:7,color:T.textDim,letterSpacing:"0.08em"}}>{d}</div>)}
      </div>
  ...
  return(<div style={{paddingBottom:90}}>
    ...
    <div style={{padding:"14px 14px 0"}}>
      <div style={{display:"flex",gap:6,marginBottom:12,flexWrap:"wrap"}}>
        <button onClick={()=>setShowCal(true)} style={{padding:"8px 14px",borderRadius:2,border:"1px solid "+T.border,background:"transparent",fontFamily:F.mono,fontSize:8,color:T.textMuted,cursor:"pointer",letterSpacing:"0.06em"}}>📅 КАЛЕНДАР</button>
        <button onClick={()=>setHideSeasonal(!hideSeasonal)} style={{padding:"8px 14px",borderRadius:2,border:hideSeasonal?"1px solid "+T.text:"1px solid "+T.border,background:hideSeasonal?T.glow:"transparent",fontFamily:F.mono,fontSize:8,color:hideSeasonal?T.text:T.textMuted,cursor:"pointer",letterSpacing:"0.06em"}}>🌿 СЕЗОНЕН</button>
        <button onClick={()=>{setBatchMode(!batchMode);setSelected(new Set());}} style={{padding:"8px 14px",borderRadius:2,border:batchMode?"1px solid "+T.text:"1px solid "+T.border,background:batchMode?T.glow:"transparent",fontFamily:F.mono,fontSize:8,color:batchMode?T.text:T.textMuted,cursor:"pointer",letterSpacing:"0.06em"}}>🧺 ПЕРАЛНЯ</button>
      </div>
      {batchMode&&<div style={{display:"flex",gap:6,marginBottom:12,padding:"10px 14px",background:T.card,border:"1px solid "+T.borderLight,borderRadius:3}}>
        <span style={{fontFamily:F.mono,fontSize:9,color:T.textMuted,flex:1,alignSelf:"center"}}>{selected.size} избрани</span>
        <button onClick={()=>batchLaundry(true)} disabled={!selected.size} style={{padding:"8px 12px",borderRadius:2,border:"1px solid "+T.border,background:"transparent",fontFamily:F.mono,fontSize:8,color:T.textMuted,cursor:"pointer",opacity:selected.size?1:0.3}}>В ПЕРАЛНЯ</button>
        <button onClick={()=>batchLaundry(false)} disabled={!selected.size} style={{padding:"8px 12px",borderRadius:2,border:"1px solid "+T.border,background:"transparent",fontFamily:F.mono,fontSize:8,color:T.textMuted,cursor:"pointer",opacity:selected.size?1:0.3}}>ЧИСТИ</button>
      </div>}
      ...
      <div style={{display:"flex",gap:0,marginBottom:12,borderBottom:"1px solid "+T.border,paddingBottom:8}}>
        {[{id:"newest",l:"Скорошни"},{id:"most_worn",l:"Най-носени"},{id:"least_worn",l:"Най-малко"},{id:"price",l:"Цена"},{id:"name",l:"А-Я"}].map(s=><button key={s.id} onClick={()=>setSort(s.id)} style={{padding:"4px 8px",border:"none",background:"none",fontFamily:F.mono,fontSize:8,color:sort===s.id?T.textSoft:T.textDim,cursor:"pointer",letterSpacing:"0.04em",textTransform:"uppercase",borderBottom:sort===s.id?"1px solid "+T.textSoft:"1px solid transparent"}}>{s.l}</button>)}
      </div>
      {filtered.length===0?<EmptyState icon={items.length===0?"👕":"🔍"} title={items.length===0?"Гардеробът ти чака":"Няма резултати"} sub={items.length===0?"НАТИСНИ ДОБАВИ ЗА НАЧАЛО":"ОПИТАЙ С ДРУГИ ФИЛТРИ"}/>:
...
function AddTab({onAdd,lastCat}){
  ...
  if(step==="photo"){return(<div style={{padding:"24px 20px"}}><p style={{...labelSt,marginBottom:3}}>НОВА ДРЕХА</p><h2 style={{fontFamily:F.serif,fontSize:26,fontWeight:300,color:T.text,margin:"0 0 28px"}}>Добави в гардероба</h2>
    ...
    {cam?(<div style={{borderRadius:3,overflow:"hidden"}}><video ref={vidRef} autoPlay playsInline muted onLoadedMetadata={e=>e.target.play().catch(()=>{})} style={{width:"100%",display:"block",background:"#000",minHeight:300,WebkitTransform:"translateZ(0)"}}/>
      ...
      <button onClick={startCam} style={{padding:"40px 20px",borderRadius:3,border:"1px dashed "+T.borderLight,background:"transparent",cursor:"pointer",textAlign:"center"}}><p style={{fontFamily:F.sans,fontSize:13,color:T.text,margin:"0 0 3px",fontWeight:500}}>Снимай с камера</p><p style={{fontFamily:F.mono,fontSize:8,color:T.textDim,margin:0,letterSpacing:"0.08em"}}>НАПРАВИ СНИМКА</p></button>
      <button onClick={()=>fileRef.current?.click()} style={{padding:"22px 20px",borderRadius:3,border:"1px dashed "+T.borderLight,background:"transparent",cursor:"pointer",textAlign:"center"}}><p style={{fontFamily:F.sans,fontSize:13,color:T.text,margin:"0 0 2px",fontWeight:500}}>Качи от галерия</p><p style={{fontFamily:F.mono,fontSize:8,color:T.textDim,margin:0,letterSpacing:"0.06em"}}>ИЗБЕРИ ФАЙЛ</p></button>
      <div style={{display:"flex",alignItems:"center",gap:14,margin:"4px 0"}}><div style={{flex:1,height:1,background:T.border}}/><span style={{fontFamily:F.mono,fontSize:8,color:T.textDim}}>ИЛИ</span><div style={{flex:1,height:1,background:T.border}}/></div>
      <button onClick={()=>{setImg(null);setStep("details");}} style={{padding:"12px",borderRadius:2,border:"1px solid "+T.border,background:"transparent",fontFamily:F.mono,fontSize:9,color:T.textDim,letterSpacing:"0.1em",cursor:"pointer",textAlign:"center"}}>БЕЗ СНИМКА</button>
  ...
  return(<div style={{padding:"24px 20px 100px"}}>
    ...
    <div style={{marginBottom:20}}><label style={labelSt}>КАТЕГОРИЯ *</label><div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:5}}>{CATEGORIES.map(c=><button key={c.id} onClick={()=>setCat(c.id)} style={{padding:"13px 5px",borderRadius:2,textAlign:"center",cursor:"pointer",border:cat===c.id?"1px solid "+T.text:"1px solid "+T.border,background:cat===c.id?T.glow:"transparent"}}><div style={{fontFamily:F.sans,fontSize:10,fontWeight:600,color:cat===c.id?T.text:T.textMuted,marginBottom:1}}>{c.label}</div><div style={{fontFamily:F.mono,fontSize:7,color:T.textDim}}>{c.sub}</div></button>)}</div></div>
...
function OutfitsTab({items,onSave,weather,blacklist,addBlacklist}){
  ...
  const shareOutfit=async(cur)=>{const text="DRESHNIK.ai Визия\n\n"+cur.items.map(i=>i.name).join(" + ")+"\n\n"+getExplanation(cur,occ);if(navigator.share){try{await navigator.share({title:"DRESHNIK.ai",text});toast("Споделено ✓","success");}catch(e){console.warn(e);}}else{await navigator.clipboard?.writeText(text);toast("Копирано ✓","success");}};
  const cur=results[idx];
  return(<div style={{padding:"24px 20px 100px"}}>
    <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:20}}><div><p style={{fontFamily:F.mono,fontSize:8,color:T.textDim,letterSpacing:"0.16em",marginBottom:3}}>AI СТИЛИСТ</p><h2 style={{fontFamily:F.serif,fontSize:26,fontWeight:300,color:T.text,margin:0}}>Генерирай визия</h2></div>
    {items.length>=2&&<button onClick={surprise} style={{padding:"8px 14px",borderRadius:2,border:"1px solid "+T.borderLight,background:"transparent",cursor:"pointer",fontFamily:F.mono,fontSize:9,color:T.textMuted,letterSpacing:"0.06em"}}>🎲 ИЗНЕНАДА</button>}</div>
    {weather&&<div style={{display:"flex",alignItems:"center",gap:8,marginBottom:18,padding:"10px 14px",border:"1px solid "+T.border,borderRadius:2}}><span style={{fontSize:16}}>{weather.icon}</span><span style={{fontFamily:F.mono,fontSize:10,color:T.textMuted}}>{weather.temp}°C · {weather.desc}{weather.rain?" — режим за дъжд":""}</span></div>}
    ...
    {anim&&<div style={{textAlign:"center",padding:44}}><p style={{fontFamily:F.mono,fontSize:9,color:T.textDim,letterSpacing:"0.12em",animation:"pulse 1s ease-in-out infinite"}}>АНАЛИЗИРАМ КОМБИНАЦИИ</p></div>}
    {!anim&&cur&&<div style={{animation:"fadeIn .35s ease"}} onTouchStart={onTS} onTouchMove={onTM} onTouchEnd={onTE}>
      ...
      <p style={{fontFamily:F.mono,fontSize:8,color:T.textDim,textAlign:"center",letterSpacing:"0.1em",marginBottom:12}}>ПЛЪЗНИ ЗА СЛЕДВАЩА</p>
      ...
      <div style={{display:"flex",gap:6,marginTop:8}}><button onClick={()=>shareOutfit(cur)} style={{flex:1,padding:12,borderRadius:2,border:"1px solid "+T.border,background:"transparent",fontFamily:F.mono,fontSize:9,color:T.textMuted,cursor:"pointer",letterSpacing:"0.06em"}}>СПОДЕЛИ</button>
      <button onClick={()=>{const key=cur.items.map(i=>i.id).sort().join(",");addBlacklist(key);next();toast("Комбинацията няма да се предлага","info");}} style={{flex:1,padding:12,borderRadius:2,border:"1px solid "+T.danger,background:"transparent",fontFamily:F.mono,fontSize:9,color:T.dangerText,cursor:"pointer",letterSpacing:"0.06em"}}>НЕ ПРЕДЛАГАЙ</button></div>
    </div>}
  </div>);
}
...
function SavedTab({saved,items,onDelete}){
  ...
  return(<div style={{padding:"24px 20px 100px"}}><p style={{fontFamily:F.mono,fontSize:8,color:T.textDim,letterSpacing:"0.16em",marginBottom:3}}>КОЛЕКЦИЯ</p><h2 style={{fontFamily:F.serif,fontSize:26,fontWeight:300,color:T.text,margin:"0 0 14px"}}>Запазени визии</h2>
...
function InsightsTab({items,saved}){
  ...
  const Section=({title,children})=>(<div style={{background:T.card,border:"1px solid "+T.border,borderRadius:2,padding:16,marginBottom:14,boxShadow:T.shadow}}><p style={{fontFamily:F.mono,fontSize:8,color:T.textDim,letterSpacing:"0.12em",margin:"0 0 12px"}}>{title}</p>{children}</div>);
  return(<div style={{padding:"24px 20px 100px"}}><p style={{fontFamily:F.mono,fontSize:8,color:T.textDim,letterSpacing:"0.16em",marginBottom:3}}>АНАЛИЗ НА ГАРДЕРОБА</p><h2 style={{fontFamily:F.serif,fontSize:26,fontWeight:300,color:T.text,margin:"0 0 22px"}}>Статистика</h2>
    <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:6,marginBottom:18}}>{[{n:items.length,l:"ДРЕХИ"},{n:totalWears,l:"НОСЕНИЯ"},{n:saved.length,l:"ВИЗИИ"},{n:totalValue>0?totalValue+"лв":"—",l:"СТОЙНОСТ"}].map((d,i)=><div key={i} style={{background:T.card,border:"1px solid "+T.border,borderRadius:2,padding:"16px 8px",textAlign:"center",boxShadow:T.shadow}}><p style={{fontFamily:F.serif,fontSize:24,color:T.text,margin:"0 0 2px",fontWeight:300}}>{d.n}</p><p style={{fontFamily:F.mono,fontSize:7,color:T.textDim,letterSpacing:"0.1em",margin:0}}>{d.l}</p></div>)}</div>
    {/* Achievements */}
    <Section title="🏆 ПОСТИЖЕНИЯ">{achievements.map((a,i)=><div key={i} style={{display:"flex",alignItems:"center",gap:10,padding:"8px 0",borderBottom:i<achievements.length-1?"1px solid "+T.border:"none",opacity:a.done?1:0.4}}><span style={{fontSize:18}}>{a.icon}</span><div style={{flex:1}}><p style={{fontFamily:F.sans,fontSize:11,color:T.text,margin:0}}>{a.name}</p><p style={{fontFamily:F.mono,fontSize:8,color:T.textDim,margin:"2px 0 0"}}>{a.desc}</p></div><span style={{fontFamily:F.mono,fontSize:9,color:a.done?"#88CC88":T.textDim}}>{a.done?"✓":a.progress}</span></div>)}</Section>
    {totalValue>0&&<Section title="РАЗХОДИ ПО КАТЕГОРИЯ">{catBreakdown.filter(c=>c.value>0).map(c=><div key={c.id} style={{marginBottom:8}}><div style={{display:"flex",justifyContent:"space-between",marginBottom:3}}><span style={{fontFamily:F.sans,fontSize:11,color:T.textSoft}}>{c.label}</span><span style={{fontFamily:F.mono,fontSize:9,color:T.textDim}}>{c.value}лв ({c.count})</span></div><div style={{height:3,background:T.border,borderRadius:2,overflow:"hidden"}}><div style={{width:(c.value/totalValue*100)+"%",height:"100%",background:T.text,borderRadius:2}}/></div></div>)}<div style={{marginTop:10,paddingTop:10,borderTop:"1px solid "+T.border}}><div style={{display:"flex",justifyContent:"space-between"}}><span style={{fontFamily:F.mono,fontSize:9,color:T.textMuted}}>СР. ЦЕНА/НОСЕНЕ</span><span style={{fontFamily:F.serif,fontSize:18,color:T.text}}>{avgCPW} лв</span></div></div></Section>}
    {bestCPW.length>0&&<Section title="НАЙ-ДОБРА ЦЕНА/НОСЕНЕ">{bestCPW.map((item,i)=><div key={item.id} style={{display:"flex",alignItems:"center",gap:10,padding:"6px 0",borderBottom:i<bestCPW.length-1?"1px solid "+T.border:"none"}}>{item.image?<img src={item.image} alt="" style={{width:28,height:28,borderRadius:2,objectFit:"cover"}}/>:<div style={{width:28,height:28,borderRadius:2,background:T.surface}}/>}<span style={{fontFamily:F.sans,fontSize:11,color:T.text,flex:1}}>{item.name}</span><span style={{fontFamily:F.mono,fontSize:9,color:T.textDim}}>{item.cpw}лв/носене</span></div>)}</Section>}
    <Section title="КАТЕГОРИИ">{catBreakdown.map(c=><div key={c.id} style={{marginBottom:8}}><div style={{display:"flex",justifyContent:"space-between",marginBottom:3}}><span style={{fontFamily:F.sans,fontSize:11,color:T.textSoft}}>{c.label}</span><span style={{fontFamily:F.mono,fontSize:9,color:T.textDim}}>{c.count}</span></div><div style={{height:3,background:T.border,borderRadius:2,overflow:"hidden"}}><div style={{width:(c.count/maxCat*100)+"%",height:"100%",background:T.text,borderRadius:2}}/></div></div>)}</Section>
    {topColors.length>0&&<Section title="ЦВЕТОВА ПАЛИТРА"><div style={{display:"flex",gap:3,marginBottom:10,height:28}}>{topColors.map(([hex],i)=><div key={i} style={{flex:1,background:hex,borderRadius:2,border:hex==="#FFFFFF"?"1px solid "+T.border:"none"}}/>)}</div><div style={{display:"flex",gap:5,flexWrap:"wrap"}}>{topColors.map(([hex,count])=><span key={hex} style={{fontFamily:F.mono,fontSize:7,color:T.textDim}}>{getColorName(hex)} ×{count}</span>)}</div></Section>}
...
function AuthScreen(){
  ...
  return(<div style={{minHeight:"100dvh",background:T.bg,display:"flex",flexDirection:"column",justifyContent:"center",padding:"0 28px"}}>
    ...
    {mode!=="reset"&&<><div style={{display:"flex",alignItems:"center",gap:14,margin:"8px 0 20px"}}><div style={{flex:1,height:1,background:T.border}}/><span style={{fontFamily:F.mono,fontSize:8,color:T.textDim}}>ИЛИ</span><div style={{flex:1,height:1,background:T.border}}/></div>
    ...
    <div style={{display:"flex",justifyContent:"center",gap:16,marginTop:8}}>
      {mode==="login"&&<><button onClick={()=>{setMode("register");setError(null);}} style={{background:"none",border:"none",color:T.textMuted,fontFamily:F.mono,fontSize:9,cursor:"pointer",letterSpacing:"0.06em"}}>СЪЗДАЙ АКАУНТ</button><button onClick={()=>{setMode("reset");setError(null);}} style={{background:"none",border:"none",color:T.textDim,fontFamily:F.mono,fontSize:9,cursor:"pointer",letterSpacing:"0.06em"}}>ЗАБРАВЕНА ПАРОЛА</button></>}
      {mode!=="login"&&<button onClick={()=>{setMode("login");setError(null);setResetSent(false);}} style={{background:"none",border:"none",color:T.textMuted,fontFamily:F.mono,fontSize:9,cursor:"pointer",letterSpacing:"0.06em"}}>ОБРАТНО КЪМ ВХОД</button>}
    </div>
  </div>);
}
...
function ProfileTab(){
  ...
  return(<div style={{padding:"24px 20px 100px"}}>
    ...
    <Section title="БИО">
      {editBio?<div><textarea value={bioVal} onChange={e=>setBioVal(e.target.value)} rows={3} style={{...inputSt,resize:"none",marginBottom:8}} placeholder="Разкажи за стила си..."/><button onClick={()=>{saveProfile({bio:bioVal});setEditBio(false);toast("Запазено","success");}} style={{padding:"8px 16px",borderRadius:2,border:"1px solid "+T.text,background:"transparent",color:T.text,fontFamily:F.mono,fontSize:9,cursor:"pointer"}}>ЗАПАЗИ</button></div>:
    ...
    <Section title="НАСТРОЙКИ">
      ...
      <div style={{padding:"12px 0",borderBottom:"1px solid "+T.border}}>
        <p style={{fontFamily:F.mono,fontSize:7,color:T.textDim,letterSpacing:"0.12em",marginBottom:8}}>ВАЛУТА</p>
    ...
    <Section title="АКАУНТ">
      <button onClick={handleExport} style={{width:"100%",padding:13,borderRadius:2,border:"1px solid "+T.border,background:"transparent",fontFamily:F.mono,fontSize:9,color:T.textMuted,cursor:"pointer",letterSpacing:"0.06em",marginBottom:8}}>ЕКСПОРТИРАЙ ДАННИ (JSON)</button>
      <button onClick={()=>confirm("Искаш ли да излезеш от акаунта?",()=>logout())} style={{width:"100%",padding:13,borderRadius:2,border:"1px solid "+T.border,background:"transparent",fontFamily:F.mono,fontSize:9,color:T.textMuted,cursor:"pointer",letterSpacing:"0.06em",marginBottom:8}}>ИЗЛЕЗ</button>
      {!showDelete?<button onClick={()=>setShowDelete(true)} style={{width:"100%",padding:13,borderRadius:2,border:"1px solid "+T.danger,background:"transparent",fontFamily:F.mono,fontSize:9,color:T.dangerText,cursor:"pointer",letterSpacing:"0.06em"}}>ИЗТРИЙ АКАУНТ</button>:
      <div style={{border:"1px solid "+T.danger,borderRadius:3,padding:16}}>
        ...
        <div style={{display:"flex",gap:8}}><button onClick={()=>{setShowDelete(false);setDelPass("");}} style={{flex:1,padding:12,borderRadius:2,border:"1px solid "+T.border,background:"transparent",fontFamily:F.mono,fontSize:9,color:T.textMuted,cursor:"pointer"}}>ОТКАЗ</button>
        <button onClick={handleDelete} disabled={!isGoogle&&!delPass} style={{flex:1,padding:12,borderRadius:2,border:"none",background:T.dangerText,color:"#fff",fontFamily:F.mono,fontSize:9,cursor:"pointer",opacity:busy?0.5:1}}>ИЗТРИЙ</button></div>
      </div>}
    </Section>
    <p style={{fontFamily:F.mono,fontSize:8,color:T.textDim,textAlign:"center",letterSpacing:"0.08em",marginTop:20}}>DRESHNIK.AI v8 · MADE IN BULGARIA 🇧🇬</p>
  </div>);
}
