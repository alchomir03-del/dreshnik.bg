...
// === Themes ===
const THEMES={
  dark:{bg:"#060606",surface:"#0C0C0C",card:"#0D0D0D",border:"#1A1A1A",borderLight:"#222222",text:"#F0ECE4",textSoft:"#BBBBAA",textMuted:"#777766",textDim:"#3A3A3A",glow:"rgba(240,236,228,0.03)",overlay:"rgba(0,0,0,0.85)",danger:"#221111",dangerText:"#CC4444",imgOpacity:0.88,shadow:"none",accent:"#F0ECE4"},
...
function ToastOverlay({toasts}){
  ...
  return(<div style={{position:"fixed",top:"env(safe-area-inset-top, 12px)",left:0,right:0,zIndex:999,display:"flex",flexDirection:"column",alignItems:"center",gap:6,padding:"12px 20px",pointerEvents:"none"}}>
    {toasts.map(t=><div key={t.id} style={{background:T.surface,border:"1px solid rgba(255,255,255,0.06)",borderRadius:100,padding:"10px 24px",fontFamily:F.serif,fontSize:13,fontWeight:400,color:t.type==="error"?T.dangerText:T.text,boxShadow:"0 4px 20px rgba(0,0,0,0.3)",animation:"fadeInUp 0.3s cubic-bezier(0.16, 1, 0.3, 1)",pointerEvents:"auto",maxWidth:340,textAlign:"center",letterSpacing:"0.01em"}}>{t.msg}</div>)}
  </div>);
}
...
// === Shared UI ===
function Pill({children,active,onClick,small}){const T=useTheme();return(<button onClick={onClick} style={{padding:small?"6px 14px":"8px 18px",borderRadius:100,border:active?"1px solid "+T.text:"1px solid rgba(255,255,255,0.06)",background:active?T.text:"rgba(255,255,255,0.03)",color:active?T.bg:T.textMuted,fontFamily:F.serif,fontSize:small?12:13,fontWeight:active?500:300,cursor:"pointer",transition:"all 0.4s cubic-bezier(0.16, 1, 0.3, 1)",whiteSpace:"nowrap",letterSpacing:"0.02em"}}>{children}</button>);}
function Logo({size=28,animate}){const T=useTheme();return(<span style={{fontFamily:F.serif,fontSize:size,color:"#F0ECE4",animation:animate?"pulse 2s ease-in-out infinite":"none",letterSpacing:"-0.02em"}}><span style={{fontWeight:500}}>DRESHNIK</span><span style={{fontWeight:300,color:"#3A3A3A"}}>.bg</span></span>);}
function ColorBar({items}){const all=items.flatMap(i=>i.colors||[]);if(!all.length)return null;return(<div style={{display:"flex",height:2,borderRadius:1,overflow:"hidden",gap:2,opacity:0.7}}>{all.map((c,i)=><div key={i} style={{flex:1,background:c}}/>)}</div>);}
function WeatherBar({weather}){const T=useTheme(),t=useT();if(!weather)return null;const desc=t(weather.desc);return(<div style={{margin:"14px 20px 0",display:"inline-flex",alignItems:"center",gap:10,padding:"10px 18px",background:"rgba(255,255,255,0.03)",border:"1px solid rgba(255,255,255,0.06)",borderRadius:100}}><span style={{fontSize:16}}>{weather.icon}</span><span style={{fontFamily:F.sans,fontSize:12,color:T.textMuted}}>{weather.temp}° {desc}{weather.city?" / "+weather.city:""}</span>{weather.rainChance>0&&<span style={{fontFamily:F.mono,fontSize:10,color:T.textDim}}>☂ {weather.rainChance}%</span>}</div>);}
function EmptyState({icon,title,sub}){const T=useTheme();return(<div style={{textAlign:"center",padding:"60px 24px"}}><div style={{fontSize:32,marginBottom:20,opacity:0.3}}>{icon}</div><p style={{fontFamily:F.serif,fontSize:22,color:T.text,fontWeight:300,marginBottom:8,letterSpacing:"-0.01em"}}>{title}</p><p style={{fontFamily:F.mono,fontSize:9,color:T.textDim,letterSpacing:"0.16em",textTransform:"uppercase"}}>{sub}</p></div>);}
...
function Nav({tab,setTab,count,theme,toggleTheme}){
  ...
  return(<div style={{background:T.bg,borderBottom:"1px solid "+T.border,position:"sticky",top:0,zIndex:100,paddingTop:"env(safe-area-inset-top, 0px)"}}>
    <div style={{padding:"20px 22px 0"}}><div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:18}}>
      <Logo size={24}/>
      <div style={{display:"flex",alignItems:"center",gap:14}}>
        ...
        <span style={{fontFamily:F.mono,fontSize:9,color:T.textDim,letterSpacing:"0.12em"}}>{count}</span>
      </div>
    </div></div>
    <div style={{display:"flex",padding:"0 8px",overflowX:"auto"}}>{tabs.map(tb=>(<button key={tb.id} onClick={()=>setTab(tb.id)} style={{flex:1,padding:"12px 0 14px",border:"none",background:"none",borderBottom:tab===tb.id?"1px solid "+T.text:"1px solid transparent",color:tab===tb.id?T.text:T.textDim,fontFamily:F.serif,fontSize:tab===tb.id?13:12,fontWeight:tab===tb.id?500:300,cursor:"pointer",transition:"all 0.4s cubic-bezier(0.16, 1, 0.3, 1)",whiteSpace:"nowrap",letterSpacing:"0.02em"}}>{tb.label}</button>))}</div>
  </div>);
}
...
function ItemCard({item,onClick,onDoubleTap,idx=0}){
  ...
  return(<div onClick={handleTap} style={{background:T.card,borderRadius:4,overflow:"hidden",cursor:"pointer",border:"1px solid "+T.border,opacity:item.inLaundry?0.4:1,transition:"all 0.4s cubic-bezier(0.16, 1, 0.3, 1)",animation:"fadeInUp "+(0.12+idx*0.04)+"s ease both"}}>
    <div style={{position:"relative",paddingTop:"130%",background:T.surface}}>
      ...
      <div style={{position:"absolute",inset:0,display:"flex",alignItems:"center",justifyContent:"center",background:"linear-gradient(145deg, "+(item.colors?.[0]||"#222")+"15 0%, #0A0A0A 100%)"}}><div style={{width:36,height:36,borderRadius:"50%",background:item.colors?.[0]||"#222",opacity:0.4,border:item.colors?.[0]==="#000000"?"1px solid #222":"none"}}/></div>}
      <div style={{position:"absolute",bottom:0,left:0,right:0,height:60,background:"linear-gradient(0deg,rgba(0,0,0,0.6) 0%,transparent 100%)"}}/>
      <span style={{position:"absolute",top:8,left:8,fontFamily:F.mono,fontSize:8,color:"rgba(255,255,255,0.35)",letterSpacing:"0.12em",textTransform:"uppercase"}}>{cat?.label}{item.inLaundry?" · 🧺":""}</span>
      {item.favorite&&<span style={{position:"absolute",top:8,right:8,fontSize:11,color:"rgba(240,236,228,0.5)"}}>♥</span>}
      {item.capsule&&<div style={{position:"absolute",top:8,right:item.favorite?26:8,width:8,height:8,borderRadius:"50%",background:"#F0ECE4",opacity:0.5}}/>}
      {item.colors?.length>0&&<div style={{position:"absolute",bottom:8,right:8,display:"flex",gap:2}}>{item.colors.slice(0,3).map((c,i)=><div key={i} style={{width:8,height:8,borderRadius:1,background:c,border:"1px solid rgba(255,255,255,0.06)"}}/>)}</div>}
      {(item.wearCount||0)>0&&<span style={{position:"absolute",bottom:8,left:8,fontFamily:F.mono,fontSize:9,color:"rgba(255,255,255,0.3)"}}>{item.wearCount}×</span>}
    </div>
    <div style={{padding:"12px 12px 13px"}}>
      ...
      <p style={{fontFamily:F.mono,fontSize:8,color:T.textDim,margin:"4px 0 0",letterSpacing:"0.08em",textTransform:"uppercase"}}>{item.brand||STYLES[STYLE_IDS.indexOf(item.style)]||""}</p>
    </div>
  </div>);
}
      ...
      <div style={{padding:"20px 22px 34px"}}>
        {!editing?<>
          <div style={{display:"flex",gap:6,marginBottom:10,flexWrap:"wrap"}}>
            ...
            {item.capsule&&<span style={{border:"1px solid #665500",padding:"3px 10px",borderRadius:2,fontFamily:F.mono,fontSize:10,color:"#CCAA00",letterSpacing:"0.08em"}}>{t("КАПСУЛА")}</span>}
            {item.inLaundry&&<span style={{border:"1px solid #445566",padding:"3px 10px",borderRadius:2,fontFamily:F.mono,fontSize:10,color:"#88AACC",letterSpacing:"0.08em"}}>{t("В ПЕРАЛНЯ")}</span>}
          </div>
        ...
        <div>
          ...
          <label style={labelSt}>{t("ИМЕ")}</label><input value={editData.name||""} onChange={e=>setEditData(p=>({...p,name:e.target.value}))} style={inputSt}/>
...
function OOTDCard({items,weather,onDismiss,calendar}){
  const T=useTheme(),t=useT(),[combo,setCombo]=useState(null),[show,setShow]=useState(true);
  ...
  return(<div style={{margin:"14px 14px 0",background:T.card,border:"1px solid "+T.borderLight,borderRadius:4,overflow:"hidden",boxShadow:T.shadow}}>
    <div style={{padding:"16px 18px 14px",display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
      <div><p style={{fontFamily:F.mono,fontSize:10,color:T.textDim,letterSpacing:"0.16em",margin:"0 0 4px"}}>ВИЗИЯ НА ДЕНЯ</p><p style={{fontFamily:F.serif,fontSize:18,color:T.text,fontWeight:400,margin:0}}>{wornToday?t("Вече отбеляза днешната визия"):t("Предложение за днес")}</p></div>
...
function CalendarModal({calendar,items,onClose}){
  const T=useTheme(),t=useT(),[month,setMonth]=useState(()=>{const d=new Date();return{y:d.getFullYear(),m:d.getMonth()};});
  ...
  return(<div style={{paddingBottom:90}}>
    ...
    <div style={{padding:"20px 20px 0"}}>
      ...
      <div style={{display:"flex",gap:8,marginBottom:16}}>
        <button onClick={()=>setShowCal(true)} style={{padding:"8px 16px",borderRadius:100,border:"1px solid rgba(255,255,255,0.06)",background:"rgba(255,255,255,0.03)",fontFamily:F.serif,fontSize:12,color:T.textMuted,cursor:"pointer",fontWeight:300,letterSpacing:"0.02em",transition:"all 0.3s"}}>{t("Календар")}</button>
        <button onClick={()=>setHideSeasonal(!hideSeasonal)} style={{padding:"8px 16px",borderRadius:100,border:hideSeasonal?"1px solid "+T.text:"1px solid rgba(255,255,255,0.06)",background:hideSeasonal?"rgba(255,255,255,0.03)":"rgba(255,255,255,0.03)",fontFamily:F.serif,fontSize:12,color:hideSeasonal?T.text:T.textMuted,cursor:"pointer",fontWeight:hideSeasonal?500:300,letterSpacing:"0.02em",transition:"all 0.3s"}}>{t("Сезонен")}</button>
        <button onClick={()=>{setBatchMode(!batchMode);setSelected(new Set());}} style={{padding:"8px 16px",borderRadius:100,border:batchMode?"1px solid "+T.text:"1px solid rgba(255,255,255,0.06)",background:"rgba(255,255,255,0.03)",fontFamily:F.serif,fontSize:12,color:batchMode?T.text:T.textMuted,cursor:"pointer",fontWeight:batchMode?500:300,letterSpacing:"0.02em",transition:"all 0.3s"}}>{t("Пералня")}</button>
      </div>
      {batchMode&&<div style={{display:"flex",gap:8,marginBottom:14,padding:"12px 16px",background:"rgba(255,255,255,0.02)",borderLeft:"2px solid "+T.text,alignItems:"center"}}>
        <span style={{fontFamily:F.serif,fontSize:13,color:T.textSoft,flex:1,fontWeight:300}}>{selected.size} {t("избрани")}</span>
        <button onClick={()=>batchLaundry(true)} disabled={!selected.size} style={{padding:"6px 14px",borderRadius:100,border:"1px solid rgba(255,255,255,0.06)",background:"rgba(255,255,255,0.03)",fontFamily:F.serif,fontSize:11,color:T.textMuted,cursor:"pointer",opacity:selected.size?1:0.3}}>{t("В пералня")}</button>
        <button onClick={()=>batchLaundry(false)} disabled={!selected.size} style={{padding:"6px 14px",borderRadius:100,border:"1px solid rgba(255,255,255,0.06)",background:"rgba(255,255,255,0.03)",fontFamily:F.serif,fontSize:11,color:T.textMuted,cursor:"pointer",opacity:selected.size?1:0.3}}>{t("Чисти")}</button>
      </div>}
      <input value={search} onChange={e=>setSearch(e.target.value)} placeholder={t("Търси по име, марка, таг...")} style={{width:"100%",padding:"12px 18px",borderRadius:100,border:"1px solid rgba(255,255,255,0.06)",fontSize:12,fontFamily:F.sans,color:T.text,background:"rgba(255,255,255,0.03)",boxSizing:"border-box",marginBottom:14}}/>
      ...
      {/* Editorial sort tabs with underline */}
      <div style={{display:"flex",gap:0,marginBottom:16,borderBottom:"1px solid "+T.border}}>
        {[{id:"newest",l:t("Скорошни")},{id:"most_worn",l:t("Най-носени")},{id:"least_worn",l:t("Най-малко")},{id:"price",l:t("Цена")},{id:"name",l:t("А-Я")}].map(s=><button key={s.id} onClick={()=>setSort(s.id)} style={{padding:"10px 12px 12px",border:"none",background:"none",fontFamily:F.serif,fontSize:sort===s.id?12:11,color:sort===s.id?T.textSoft:T.textDim,cursor:"pointer",borderBottom:sort===s.id?"1px solid "+T.text:"1px solid transparent",fontWeight:sort===s.id?500:300,transition:"all 0.3s",letterSpacing:"0.02em"}}>{s.l}</button>)}
      </div>
...
function AddTab({onAdd,lastCat}){
  ...
  useEffect(()=>()=>stopCam(),[]);
  const inputSt={width:"100%",padding:"12px 18px",borderRadius:100,border:"1px solid rgba(255,255,255,0.06)",fontSize:13,fontFamily:F.sans,color:T.text,background:"rgba(255,255,255,0.03)",boxSizing:"border-box"};
  const labelSt={fontFamily:F.mono,fontSize:9,color:T.textDim,letterSpacing:"0.2em",display:"block",marginBottom:8,textTransform:"uppercase"};
  if(step==="photo"){return(<div style={{padding:"28px 20px"}}><p style={{...labelSt,marginBottom:4}}>{t("НОВА ДРЕХА")}</p><h2 style={{fontFamily:F.serif,fontSize:34,fontWeight:300,color:T.text,margin:"0 0 8px",letterSpacing:"-0.02em"}}>{t("Добави в гардероба")}</h2><div style={{width:40,height:1,background:"linear-gradient(90deg, "+T.text+" 0%, transparent 100%)",marginBottom:32}}/>
    {err&&<div style={{borderLeft:"2px solid "+T.dangerText,padding:"10px 16px",marginBottom:16,fontFamily:F.sans,fontSize:12,color:T.textMuted}}>{err}</div>}
    {cam?(<div style={{borderRadius:4,overflow:"hidden"}}><video ref={vidRef} autoPlay playsInline muted onLoadedMetadata={e=>e.target.play().catch(()=>{})} style={{width:"100%",display:"block",background:"#000",minHeight:300,WebkitTransform:"translateZ(0)"}}/>
      ...
      <button onClick={startCam} style={{padding:"44px 20px",borderRadius:4,border:"1px solid "+T.border,background:"rgba(255,255,255,0.02)",cursor:"pointer",textAlign:"center",transition:"all 0.3s"}}><p style={{fontFamily:F.serif,fontSize:16,color:T.text,margin:"0 0 4px",fontWeight:300}}>{t("Снимай с камера")}</p><p style={{fontFamily:F.mono,fontSize:9,color:T.textDim,margin:0,letterSpacing:"0.14em"}}>{t("НАПРАВИ СНИМКА")}</p></button>
      <button onClick={()=>fileRef.current?.click()} style={{padding:"24px 20px",borderRadius:4,border:"1px solid "+T.border,background:"rgba(255,255,255,0.02)",cursor:"pointer",textAlign:"center",transition:"all 0.3s"}}><p style={{fontFamily:F.serif,fontSize:16,color:T.text,margin:"0 0 4px",fontWeight:300}}>{t("Качи от галерия")}</p><p style={{fontFamily:F.mono,fontSize:9,color:T.textDim,margin:0,letterSpacing:"0.12em"}}>{t("ИЗБЕРИ ФАЙЛ")}</p></button>
      <div style={{display:"flex",alignItems:"center",gap:14,margin:"6px 0"}}><div style={{flex:1,height:1,background:T.border}}/><span style={{fontFamily:F.mono,fontSize:9,color:T.textDim,letterSpacing:"0.16em"}}>ИЛИ</span><div style={{flex:1,height:1,background:T.border}}/></div>
  ...
  return(<div style={{padding:"28px 20px 100px"}}>
    ...
    {img&&<img src={img} alt="" style={{width:"100%",height:200,objectFit:"cover",borderRadius:4,marginBottom:24,opacity:T.imgOpacity}}/>}
    <div style={{marginBottom:22}}><label style={labelSt}>{t("ИМЕ *")}</label><input value={name} onChange={e=>setName(e.target.value)} placeholder="напр. Черна Nike Dri-FIT тениска" style={inputSt}/></div>
    <div style={{marginBottom:22}}><label style={labelSt}>{t("КАТЕГОРИЯ *")}</label><div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:6}}>{CATEGORIES.map(c=><button key={c.id} onClick={()=>setCat(c.id)} style={{padding:"14px 5px",borderRadius:4,textAlign:"center",cursor:"pointer",border:cat===c.id?"1px solid "+T.text:"1px solid "+T.border,background:cat===c.id?"rgba(255,255,255,0.03)":"transparent",transition:"all 0.3s"}}><div style={{fontFamily:F.serif,fontSize:12,fontWeight:cat===c.id?500:300,color:cat===c.id?T.text:T.textMuted,marginBottom:2}}>{c.label}</div><div style={{fontFamily:F.mono,fontSize:9,color:T.textDim,letterSpacing:"0.04em"}}>{c.sub}</div></button>)}</div></div>
    <div style={{marginBottom:22}}><label style={labelSt}>{t("ЦВЕТОВЕ * — ДО 3")}</label><div style={{display:"flex",flexWrap:"wrap",gap:5}}>{COLORS.map(([hex])=><button key={hex} onClick={()=>toggleColor(hex)} style={{width:28,height:28,borderRadius:3,background:hex,cursor:"pointer",padding:0,border:colors.includes(hex)?"2px solid "+T.text:hex==="#FFFFFF"?"1px solid "+T.border:"1px solid transparent",transform:colors.includes(hex)?"scale(1.15)":"scale(1)",transition:"all 0.2s cubic-bezier(0.16, 1, 0.3, 1)"}}/>)}</div>{colors.length>0&&<div style={{display:"flex",gap:4,marginTop:8}}>{colors.map(c=><span key={c} style={{fontFamily:F.mono,fontSize:9,border:"1px solid rgba(255,255,255,0.06)",padding:"3px 10px",borderRadius:100,color:T.textMuted,letterSpacing:"0.06em",textTransform:"uppercase"}}>{getColorName(c)}</span>)}</div>}</div>
    ...
    <div style={{marginBottom:22}}><label style={labelSt}>{t("МАРКА")}</label><div style={{display:"flex",flexWrap:"wrap",gap:4,marginBottom:8}}>{BRANDS.map(b=><button key={b} onClick={()=>setBrand(brand===b?"":b)} style={{padding:"5px 12px",borderRadius:100,border:brand===b?"1px solid "+T.text:"1px solid rgba(255,255,255,0.06)",background:brand===b?"rgba(255,255,255,0.03)":"transparent",fontFamily:F.mono,fontSize:9,color:brand===b?T.text:T.textDim,cursor:"pointer",letterSpacing:"0.04em",transition:"all 0.3s"}}>{b}</button>)}</div><input value={brand} onChange={e=>setBrand(e.target.value)} placeholder={t("Или напиши марка")} style={{...inputSt,fontSize:12}}/></div>
    <div style={{marginBottom:22}}><label style={labelSt}>{t("МАТЕРИАЛ")}</label><div style={{display:"flex",flexWrap:"wrap",gap:4}}>{MATERIALS.map(m=><button key={m} onClick={()=>setMat(mat===m?"":m)} style={{padding:"5px 12px",borderRadius:100,border:mat===m?"1px solid "+T.text:"1px solid rgba(255,255,255,0.06)",background:mat===m?"rgba(255,255,255,0.03)":"transparent",fontFamily:F.mono,fontSize:9,color:mat===m?T.text:T.textDim,cursor:"pointer",letterSpacing:"0.04em",transition:"all 0.3s"}}>{m}</button>)}</div></div>
    <div style={{display:"flex",gap:10,marginBottom:24}}><div style={{flex:1}}><label style={labelSt}>{t("ЦЕНА")}</label><input type="number" value={price} onChange={e=>setPrice(e.target.value)} placeholder="0" style={{...inputSt,fontSize:12}}/></div><div style={{flex:2}}><label style={labelSt}>{t("ТАГОВЕ — ДО 5")}</label><div style={{display:"flex",gap:6}}><input value={tagIn} onChange={e=>setTagIn(e.target.value)} onKeyDown={e=>e.key==="Enter"&&addTag()} placeholder="#лято" style={{...inputSt,fontSize:12,flex:1}}/><button onClick={addTag} style={{padding:"0 16px",borderRadius:100,border:"1px solid rgba(255,255,255,0.06)",background:"rgba(255,255,255,0.03)",color:T.textMuted,cursor:"pointer",fontFamily:F.mono,fontSize:10}}>+</button></div>{tags.length>0&&<div style={{display:"flex",gap:4,marginTop:7,flexWrap:"wrap"}}>{tags.map(t=><span key={t} onClick={()=>setTags(p=>p.filter(x=>x!==t))} style={{fontFamily:F.mono,fontSize:9,color:T.textMuted,background:"rgba(255,255,255,0.03)",padding:"4px 10px",borderRadius:100,cursor:"pointer",border:"1px solid rgba(255,255,255,0.06)"}}>#{t} ×</span>)}</div>}</div></div>
    <button onClick={save} disabled={!canSave} style={{width:"100%",padding:16,borderRadius:3,border:"none",background:canSave?T.text:T.border,color:canSave?T.bg:T.textDim,fontFamily:F.sans,fontSize:11,fontWeight:600,cursor:canSave?"pointer":"not-allowed",letterSpacing:"0.14em",textTransform:"uppercase",transition:"all 0.3s"}}>{t("Добави в гардероба")}</button>
  </div>);
}
...
function OutfitsTab({items,onSave,weather,blacklist,addBlacklist}){
  ...
  return(<div style={{padding:"28px 20px 100px"}}>
    ...
    <div style={{marginBottom:36}}>
      <p style={{fontFamily:F.mono,fontSize:9,letterSpacing:"0.3em",color:T.textDim,marginBottom:8,textTransform:"uppercase"}}>{t("Сезон")} {t(seasonLabel)} · {new Date().toLocaleDateString("bg-BG",{day:"2-digit",month:"2-digit",year:"numeric"})}</p>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-end"}}>
        ...
        {items.length>=2&&<button onClick={surprise} style={{padding:"8px 16px",borderRadius:100,border:"1px solid rgba(255,255,255,0.06)",background:"rgba(255,255,255,0.03)",cursor:"pointer",fontFamily:F.mono,fontSize:10,color:T.textMuted,letterSpacing:"0.06em"}}>{t("🎲 ИЗНЕНАДА")}</button>}
      </div>
      <div style={{width:40,height:1,background:"linear-gradient(90deg, "+T.text+" 0%, transparent 100%)",marginTop:14}}/>
    </div>

    {/* Weather pill */}
    {weather&&<div style={{display:"inline-flex",alignItems:"center",gap:10,padding:"10px 18px",background:"rgba(255,255,255,0.03)",border:"1px solid rgba(255,255,255,0.06)",borderRadius:100,marginBottom:32}}>
    ...
    {!anim&&cur&&<div style={{animation:"fadeIn .5s ease"}} onTouchStart={onTS} onTouchMove={onTM} onTouchEnd={onTE}>
      ...
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"baseline",marginBottom:20}}>
        ...
        <div style={{display:"flex",gap:3}}>{results.map((_,i)=><div key={i} style={{width:i===idx?20:4,height:2,background:i===idx?T.text:"#222",borderRadius:1,transition:"width 0.5s cubic-bezier(0.16, 1, 0.3, 1)"}}/>)}</div>
      </div>
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginBottom:24}}>
        {cur.items.map((item,i)=><div key={item.id} style={{background:T.card,borderRadius:4,overflow:"hidden",border:"1px solid "+T.border,gridRow:i===0&&cur.items.length>2?"span 2":"auto",animation:"fadeInUp "+(0.15+i*0.1)+"s ease both"}}>
          ...
          :<div style={{width:"100%",height:i===0&&cur.items.length>2?280:165,background:"linear-gradient(145deg, "+(item.colors?.[0]||"#222")+"22 0%, #0A0A0A 100%)",display:"flex",alignItems:"center",justifyContent:"center"}}>
          ...
          <div style={{padding:"12px 14px"}}>
            ...
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
              <p style={{fontFamily:F.mono,fontSize:8,color:T.textDim,margin:0,letterSpacing:"0.08em"}}>{(item.brand||"").toUpperCase()}</p>
      ...
      <div style={{padding:"22px 24px",background:"rgba(255,255,255,0.02)",borderLeft:"2px solid "+T.text,margin:"20px 0 28px"}}>
      ...
      {/* Swipe hint */}
      <p style={{fontFamily:F.mono,fontSize:9,color:T.textDim,textAlign:"center",letterSpacing:"0.16em",marginBottom:14}}>{t("ПЛЪЗНИ ЗА СЛЕДВАЩА")}</p>

      {/* Actions — editorial buttons */}
      <div style={{display:"flex",gap:8}}>
        <button onClick={next} style={{flex:1,padding:"16px 0",borderRadius:3,border:"1px solid #222",background:"transparent",fontFamily:F.sans,fontSize:11,color:T.textMuted,cursor:"pointer",letterSpacing:"0.1em",fontWeight:500,transition:"all 0.3s"}}>{t("Следваща")}</button>
...
function InsightsTab({items,saved}){
  ...
  // Achievements
  const achievements=[
    {icon:"👕",name:t("Първи стъпки"),desc:t("Добави 10 дрехи"),done:items.length>=10,progress:Math.min(items.length,10)+"/10"},
    {icon:"✦",name:t("Стилист"),desc:t("Запази 10 визии"),done:saved.length>=10,progress:Math.min(saved.length,10)+"/10"},
    {icon:"🔥",name:t("Гардеробмания"),desc:t("50 общо носения"),done:totalWears>=50,progress:Math.min(totalWears,50)+"/50"},
    {icon:"💎",name:"Капсула",desc:t("Маркирай 5 капсулни парчета"),done:capsuleItems.length>=5,progress:Math.min(capsuleItems.length,5)+"/5"},
    {icon:"🎨",name:t("Колорист"),desc:t("10+ различни цвята"),done:Object.keys(colorFreq).length>=10,progress:Object.keys(colorFreq).length+"/10"},
    {icon:"📸",name:t("Фотограф"),desc:t("Добави снимки на 20 дрехи"),done:items.filter(i=>i.image).length>=20,progress:items.filter(i=>i.image).length+"/20"},
  ];
  if(items.length===0)return(<EmptyState icon="◎" title={t("Добави дрехи за анализ")} sub={t("СТАТИСТИКАТА ЩЕ СЕ ПОЯВИ ТУК")}/>);
  const Section=({title,children})=>(<div style={{background:"rgba(255,255,255,0.02)",borderLeft:"2px solid "+T.text,padding:"18px 22px",marginBottom:16}}><p style={{fontFamily:F.mono,fontSize:9,color:T.textDim,letterSpacing:"0.16em",margin:"0 0 14px"}}>{title}</p>{children}</div>);
  return(<div style={{padding:"28px 20px 100px"}}><p style={{fontFamily:F.mono,fontSize:9,letterSpacing:"0.3em",color:T.textDim,marginBottom:8}}>{t("АНАЛИЗ НА ГАРДЕРОБА")}</p><h2 style={{fontFamily:F.serif,fontSize:34,fontWeight:300,color:T.text,margin:"0 0 6px",letterSpacing:"-0.02em"}}>{t("Статистика")}</h2><div style={{width:40,height:1,background:"linear-gradient(90deg, "+T.text+" 0%, transparent 100%)",marginBottom:24}}/>
    <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:6,marginBottom:18}}>{[{n:items.length,l:t("ДРЕХИ")},{n:totalWears,l:t("НОСЕНИЯ")},{n:saved.length,l:t("ВИЗИИ")},{n:totalValue>0?totalValue+cur:"—",l:t("СТОЙНОСТ")}].map((d,i)=><div key={i} style={{background:"rgba(255,255,255,0.02)",border:"1px solid "+T.border,borderRadius:4,padding:"18px 8px",textAlign:"center"}}><p style={{fontFamily:F.serif,fontSize:26,color:T.text,margin:"0 0 3px",fontWeight:300}}>{d.n}</p><p style={{fontFamily:F.mono,fontSize:9,color:T.textDim,letterSpacing:"0.14em",margin:0}}>{d.l}</p></div>)}</div>
    {/* Achievements */}
    <Section title={t("🏆 ПОСТИЖЕНИЯ")}>{achievements.map((a,i)=><div key={i} style={{display:"flex",alignItems:"center",gap:10,padding:"8px 0",borderBottom:i<achievements.length-1?"1px solid "+T.border:"none",opacity:a.done?1:0.4}}><span style={{fontSize:18}}>{a.icon}</span><div style={{flex:1}}><p style={{fontFamily:F.sans,fontSize:11,color:T.text,margin:0}}>{a.name}</p><p style={{fontFamily:F.mono,fontSize:10,color:T.textDim,margin:"2px 0 0"}}>{a.desc}</p></div><span style={{fontFamily:F.mono,fontSize:11,color:a.done?"#88CC88":T.textDim}}>{a.done?"✓":a.progress}</span></div>)}</Section>
    {totalValue>0&&<Section title={t("РАЗХОДИ ПО КАТЕГОРИЯ")}>{catBreakdown.filter(c=>c.value>0).map(c=><div key={c.id} style={{marginBottom:8}}><div style={{display:"flex",justifyContent:"space-between",marginBottom:3}}><span style={{fontFamily:F.sans,fontSize:11,color:T.textSoft}}>{c.label}</span><span style={{fontFamily:F.mono,fontSize:11,color:T.textDim}}>{c.value}{cur} ({c.count})</span></div><div style={{height:3,background:T.border,borderRadius:2,overflow:"hidden"}}><div style={{width:(c.value/totalValue*100)+"%",height:"100%",background:T.text,borderRadius:2}}/></div></div>)}<div style={{marginTop:10,paddingTop:10,borderTop:"1px solid "+T.border}}><div style={{display:"flex",justifyContent:"space-between"}}><span style={{fontFamily:F.mono,fontSize:11,color:T.textMuted}}>СР. ЦЕНА/НОСЕНЕ</span><span style={{fontFamily:F.serif,fontSize:18,color:T.text}}>{avgCPW} {cur}</span></div></div></Section>}
    {bestCPW.length>0&&<Section title={t("НАЙ-ДОБРА ЦЕНА/НОСЕНЕ")}>{bestCPW.map((item,i)=><div key={item.id} style={{display:"flex",alignItems:"center",gap:10,padding:"6px 0",borderBottom:i<bestCPW.length-1?"1px solid "+T.border:"none"}}>{item.image?<img src={item.image} alt="" style={{width:28,height:28,borderRadius:2,objectFit:"cover"}}/>:<div style={{width:28,height:28,borderRadius:2,background:T.surface}}/>}<span style={{fontFamily:F.sans,fontSize:11,color:T.text,flex:1}}>{item.name}</span><span style={{fontFamily:F.mono,fontSize:11,color:T.textDim}}>{item.cpw}{cur}{t("/носене")}</span></div>)}</Section>}
    <Section title={t("КАТЕГОРИИ")}>{catBreakdown.map(c=><div key={c.id} style={{marginBottom:8}}><div style={{display:"flex",justifyContent:"space-between",marginBottom:3}}><span style={{fontFamily:F.sans,fontSize:11,color:T.textSoft}}>{c.label}</span><span style={{fontFamily:F.mono,fontSize:11,color:T.textDim}}>{c.count}</span></div><div style={{height:3,background:T.border,borderRadius:2,overflow:"hidden"}}><div style={{width:(c.count/maxCat*100)+"%",height:"100%",background:T.text,borderRadius:2}}/></div></div>)}</Section>
    ...
    {mostWorn.length>0&&mostWorn[0].wearCount>0&&<Section title={t("НАЙ-НОСЕНИ")}>{mostWorn.filter(i=>i.wearCount>0).map((item,i)=><div key={item.id} style={{display:"flex",alignItems:"center",gap:10,padding:"7px 0",borderBottom:i<2?"1px solid "+T.border:"none"}}><span style={{fontFamily:F.mono,fontSize:16,color:T.textDim,width:20,textAlign:"right"}}>{i+1}</span>{item.image?<img src={item.image} alt="" style={{width:32,height:32,borderRadius:2,objectFit:"cover",opacity:T.imgOpacity}}/>:<div style={{width:32,height:32,borderRadius:2,background:T.surface}}/>}<div style={{flex:1}}><p style={{fontFamily:F.sans,fontSize:11,color:T.text,margin:0}}>{item.name}</p></div><span style={{fontFamily:F.mono,fontSize:10,color:T.textDim}}>{item.wearCount}×</span></div>)}</Section>}
    {laundryItems.length>0&&<Section title={t("В ПЕРАЛНЯ — ")+laundryItems.length}><div style={{display:"flex",gap:4,overflowX:"auto"}}>{laundryItems.map(item=><div key={item.id} style={{width:44,height:44,borderRadius:2,overflow:"hidden",flexShrink:0,border:"1px solid "+T.border,opacity:0.5}}>{item.image?<img src={item.image} alt="" style={{width:"100%",height:"100%",objectFit:"cover"}}/>:<div style={{width:"100%",height:"100%",background:T.surface}}/>}</div>)}</div></Section>}
...
function AuthScreen(){
  ...
  const inputSt={width:"100%",padding:"14px 18px",borderRadius:100,border:"1px solid rgba(255,255,255,0.06)",fontSize:14,fontFamily:F.sans,color:T.text,background:"rgba(255,255,255,0.03)",boxSizing:"border-box",marginBottom:12};
...
function ProfileTab(){
  ...
  return(<div style={{padding:"28px 20px 100px"}}>
    <div style={{display:"flex",alignItems:"center",gap:18,marginBottom:32}}>
      <div onClick={()=>fileRef.current?.click()} style={{width:72,height:72,borderRadius:"50%",overflow:"hidden",border:"1px solid "+T.border,cursor:"pointer",flexShrink:0,background:"rgba(255,255,255,0.03)",display:"flex",alignItems:"center",justifyContent:"center"}}>
